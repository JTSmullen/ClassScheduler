...
foreach ($idToDelete in $capturedIds) {
    # Note: the delete endpoint expects the ID as a path variable
    $deleteUrl = "http://localhost:8080/api/v1/schedule/delete/$idToDelete"

    try {
        Write-Host "Deleting Schedule (ID: $($idToDelete))..." -NoNewline

        # Note the method is Delete, and no body is required
